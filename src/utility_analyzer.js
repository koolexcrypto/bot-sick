...
import detectors from './uitlity_detectors/index.js';
...
async function analyzeNode(filePath, contractNode, subNode, detectionObject) {
        ...
        switch (subNodeType) {
            case types.StateVariable:
                for (let i = 0; i < detectors.length; i++) {
                    ...
                    let detectionResult = await detector.checkStateVariable(filePath, contractNode, subNode, detectionObject);
            ...
            default:
                for (let i = 0; i < detectors.length; i++) {
                    const detector = detectors[i];
                    let detectionResult = await detector.check(filePath, contractNode, subNode, detectionObject);
                    analysisNodeArr = analysisNodeArr.concat(detectionResult);
                }
        }
    } else {
        ...
        for (let s = 0; s < statements.length; s++) {
            ...
            switch (statementType) {
                case LEAF_EXPRESSION:
                    for (let i = 0; i < detectors.length; i++) {
                        const detector = detectors[i];
                        let detectionResult = await detector.checkStatement(filePath, contractNode, subNode, statement, detectionObject);
                        analysisNodeArr = analysisNodeArr.concat(detectionResult);
                    }
                    break;
                case types.IfStatement:
                    ...
                    detectionResult = await analyzeNode(filePath, contractNode, statement, detectionObject);
                    analysisNodeArr = analysisNodeArr.concat(detectionResult);
                    break;
                case types.ForLoop:
                    { // back-compatible => better to avoid this.
...
async function analyzeContract(filePath, contractNode, detectionObject) {
    // console.log(contractNode);
    let contractAnalysisArr = [];

    for (let i = 0; i < detectors.length; i++) {
        const detector = detectors[i];
        let detectionResult = await detector.checkContract(filePath, contractNode, detectionObject);
    ...
    for (let i = 0; i < subnodes.length; i++) {
        ...
        // console.log('subNode');
        // console.log(subNode);
        let analysisNodeArr = await analyzeNode(filePath, contractNode, subNode, detectionObject);
...
async function analyzeSolidityFile(filePath, detectionObject) {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (child.type == types.Contract) {
            let contractAnalysisResult = await analyzeContract(filePath, child, detectionObject);
            ...
            let analysisNodeArr = await analyzeNode(filePath, null, child, detectionObject);
